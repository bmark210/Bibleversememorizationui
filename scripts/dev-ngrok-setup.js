...
const DEV_CONFIG_FILE = path.resolve(ROOT_DIR, '.dev-ngrok-config.json');
...
function startAppServer() {
  log('▶️', 'Запуск Next.js через tunnel-профиль...');
  return spawn(
    `npx next dev --turbo --hostname 0.0.0.0 --port ${CONFIG.PORT}`,
    {
      shell: true,
      stdio: 'inherit',
      cwd: ROOT_DIR,
...
function writeDevConfig(config) {
  fs.writeFileSync(DEV_CONFIG_FILE, `${JSON.stringify(config, null, 2)}\n`, 'utf-8');
}
...
async function main() {
  ...
  writeDevConfig({
    ...
    botUrl,
    devUrl,
    staticDomain: Boolean(CONFIG.DOMAIN),
  });
