...
import { applyDailyGoalSkipAction, finalizeDailyGoalState } from "@/server/daily-goal/mutations";
...
function parseTargetKind(value: unknown): DailyGoalTargetKind {
  if (value === "my" || value === "review") return value;
  throw new Error("targetKind must be 'my' or 'review'");
}
...
export default async function handler(
  ...
  try {
    const body = (req.body ?? {}) as Partial<DailyGoalSkipRequest>;
    ...
    const externalVerseId = String(body.externalVerseId ?? "").trim();
    if (!externalVerseId) {
      throw new Error("externalVerseId is required");
    }
    ...
    const actionResult = applyDailyGoalSkipAction({
      state: reconciled.state,
      externalVerseId,
      targetKind,
    });
    ...
    if (
      ...
      (error.message.includes("must be an integer") ||
        ...
        error.message.includes("targetKind") ||
        error.message.includes("externalVerseId"))
    ) {
      return res.status(400).json({ error: error.message });
    }
    console.error("Error applying daily goal skip action:", error);
